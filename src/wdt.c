...
int doWdtReload(int argc, char *argv[]) {
	...
	if (OK != i2cMem8Write(dev, I2C_MEM_WDT_RESET, buf, 1)) {
...
int doWdtGetPeriod(int argc, char *argv[]) {
	...
	uint8_t buf[2];
	if (OK != i2cMem8Read(dev, I2C_MEM_WDT_INTERVAL_GET, buf, 2)) {
...
int doWdtSetPeriod(int argc, char *argv[]) {
	...
	if(OK != i2cMem8Write(dev, I2C_MEM_WDT_INTERVAL_SET, buff, 2)) {
...
int doWdtSetInitPeriod(int argc, char *argv[]) {
	...
	uint8_t buf[2];
	memcpy(buf, &period, 2);
	if(OK != i2cMem8Write(dev, I2C_MEM_WDT_INIT_INTERVAL_SET, buf, 2)) {
		printf("Fail to write watchdog period!\n");
		return ERR;
	}
	return OK;
...
int doWdtGetInitPeriod(int argc, char *argv[]) {
	...
	uint8_t buf[2];
	if(OK != i2cMem8Read(dev, I2C_MEM_WDT_INIT_INTERVAL_GET, buf, 2)) {
		printf("Fail to read watchdog period!\n");
		return ERR;
	}
	uint16_t period;
...
int doWdtGetOffPeriod(int argc, char *argv[])
{
	...
	uint8_t buf[4];
	if (OK != i2cMem8Read(dev, I2C_MEM_WDT_POWER_OFF_INTERVAL_GET, buf, 4)) {
		printf("Fail to read watchdog period!\n");
		return ERR;
	}
	uint32_t period;
...
int doWdtSetOffPeriod(int argc, char *argv[]) {
	...
	uint8_t buf[4];
	memcpy(buf, &period, 4);
	if (OK != i2cMem8Write(dev, I2C_MEM_WDT_POWER_OFF_INTERVAL_SET, buf, 4)) {
		printf("Fail to write watchdog period!\n");
		return ERR;
	}
	return OK;
...
int doWdtGetResetCount(int argc, char *argv[]) {
	...
	uint8_t buf[2];
	if (OK != i2cMem8Read(dev, I2C_MEM_WDT_RESET_COUNT, buf, 2))
	{
...
int doWdtClearResetCount(int argc, char *argv[]) {
	...
	if (OK != i2cMem8Write(dev, I2C_MEM_WDT_CLEAR_RESET_COUNT, buf, 1))
	{
